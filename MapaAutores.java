...
public class MapaAutores {
	...
	// Lee los datos del fichero que relaciona el identificador de cada autor con su
	// nombre.
	// y los guarda en un mapa que tiene como clave el identificador del autor y el
	// propio autor como valor.
	public void cargarAutores(String nombre) { // Leer del fichero los codigos de AUTORES
		try { // con el Scanner
			...
			while (entrada.hasNextLine()) { // Mientras en el fichero exista una siguiente linea que leer
				...
				String info[] = linea.split("\\s+#\\s+"); // Guardamos en la variable info el código y el nombre del
															// autor
